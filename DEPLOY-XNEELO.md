# Putting Equinix Academy on centenarynetworks.com/equinixacademy

Equinix Academy is the same application as SPS Academy, in a second folder on the same
...
different company's learner records. We write to `public_html/equinixacademy/` and nowhere
...
```
cd ../sps
php tools/sync-backend.php --check ../Equinix     # what has drifted
php tools/sync-backend.php --apply ../Equinix     # write it
```

An edit made here to any of those files is overwritten by the next sync, without warning
and without a merge. If Equinix needs something SPS does not, it becomes a key in
`lib/brand.php` and a `brand('key')` in the shared page — added in SPS, synced back out.

**What is genuinely Equinix's**, and is never touched by the sync:
...
| Secret | Value |
|---|---|
| `EQUINIX_FTP_SERVER` | `centenarynetworks.com` |
| `EQUINIX_FTP_USERNAME` | the Xneelo SFTP user |
| `EQUINIX_FTP_PASSWORD` | its password |

The same Xneelo account as SPS today. If Xneelo's panel will give a second FTP user whose
home is `public_html/equinixacademy`, use that instead — then this repository's credential
...
Over SFTP, create `~/private/equinixacademy-config.php` — **outside** `public_html`, and
...
`public_html/equinixacademy/` looks for `equinixacademy-config.php` and will accept nothing
else. If it is missing the site refuses to start and says which file it wanted.

```php
'tenant'    => 'equinix',          // NOT another academy's slug. This line is the entire separation.
...
`https://centenarynetworks.com/equinixacademy/setup`, run it — then **come back and empty
...
GitHub → Actions → **Deploy Equinix Academy** → Run workflow. Pushing does not deploy.
...
- `/contact` says "Equinix Academy" and does **not** carry SPS branding — this is the one
...
1. `https://centenarynetworks.com/equinixacademy/` — the Equinix homepage, teal, Equinix logo.
2. `/equinixacademy/contact` — send a real registration through it.
3. `/equinixacademy/admin` — sign in, and confirm that registration is the **only** thing
   listed. If SPS's registrations appear, stop: the configuration file is wrong.
4. `/equinixacademy/phpcheck.php` — should be a 404 once `setup_token` is empty.
...
- **The phone number in `lib/brand.php` is a placeholder** carried over from the static
  site. It needs Equinix's real switchboard before anyone is told the site is finished.
- **The company's registered name needs confirming.** The static site used
  "Equinix Utilities (Pty) Ltd" on one page and "Equinix — Digital Utility" on another.
...
- **The privacy notice is a draft** and names SPS's outstanding items too: each company's
  ...
  period. It should be read by Equinix before the site is announced.
