...
(function(){
  ...
  var SKILLS=[
    ...
    {k:'data',  n:'Data &amp; spreadsheets',          l:'data and spreadsheets',
     d:'Capacity, exports, formulas — getting numbers into a usable shape'},
    {k:'tech',  n:'Data centre &amp; technical systems',l:'data centre and technical systems',
     d:'Power, cooling, cabling, fault-finding, the DCIM platform'},
    {k:'comms', n:'Customer &amp; written comms',     l:'customer and written communication',
     d:'Explaining a ticket, a fault or an outcome so it lands first time'},
  ...
  var ROLES=[
    {k:'dctech',  n:'Data Centre Technician',        t:{ai:3,data:3,tech:4,comms:2,report:2,admin:3,resp:3,lead:1}},
    {k:'field',   n:'Field Engineering',             t:{ai:3,data:2,tech:3,comms:3,report:2,admin:3,resp:3,lead:2}},
    {k:'netops',  n:'Network Operations',            t:{ai:3,data:4,tech:4,comms:3,report:3,admin:3,resp:4,lead:1}},
    {k:'care',    n:'Customer Support',              t:{ai:4,data:2,tech:2,comms:4,report:2,admin:3,resp:4,lead:1}},
  ...
  var BACKGROUNDS=[
    {k:'infra',n:'Data centres / IT infrastructure'},
  ...
  /* Course catalogue with the skills each one moves. Slugs match course.html.
     ...
     nothing in digital infrastructure, so recommending them would be noise. */
  var COURSES=[
  ...
  var PROF=(typeof window!=='undefined'&&window.EquinixProfile)||null;
  var prefilled=false;
  if(PROF&&PROF.exists()){
    var pp=PROF.get();
  ...
  function analyse(a){
    ...
    var techRole=['dctech','field','netops','datasys'].indexOf(a.role)>=0;
  ...
  function rplCase(rows,qual,yrs){
    ...
    if(byK.tech&&byK.tech.self>=3) ev.push('Work orders, change records and hand-over sheets from installs and repairs you signed off');
    if(byK.data&&byK.data.self>=3) ev.push('Capacity reports, exports or spreadsheets you built that other people rely on');
  ...
  function renderStep1(){
    return '<div class="sg-card">'+
      ...
      '<p class="sg-intro">This shapes what we compare you against — a target for a data centre technician is not the target for someone in network operations.</p>'+
      (prefilled?'<p class="prefill-note">Filled in from <a href="profile">your profile</a>. Change anything that\'s moved on since.</p>':'')+
      '<div class="two">'+
        sel('sgRole',ROLES,'Your role at Equinix')+
